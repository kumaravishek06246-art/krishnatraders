...
export default function App() {
  ...
  const [products, setProducts] = useState([
    ...
    { id: 2, name: "TATA Tiscon", price: "₹75/kg", category: "Sariya & Iron" }
  ]);
  ...
  const delProd = (id) => setProducts(products.filter(p => p.id !== id));
    ...
    <div style={{ background: "#0f172a", color: "#fff", minHeight: "100vh", padding: "20px", fontFamily: "sans-serif" }}>
      ...
      {page === "store" ? (
        <div>
          {products.map(p => (
            <div key={p.id} style={{ background: "#1e293b", padding: "15px", marginBottom: "10px", borderRadius: "8px" }}>
              <h3>{p.name}</h3>
              <p style={{ color: "#34d399" }}>{p.price}</p>
              <button onClick={() => handleWA(p)} style={{ width: "100%", background: "#059669", border: "none", color: "#fff", padding: "10px", marginTop: "10px" }}>WhatsApp</button>
            </div>
          ))}
        </div>
      ) : (
        <div>
          {!user ? (
            <button onClick={() => alert("Admin Panel requires Firebase Auth setup")} style={{ background: "#059669", width: "100%", padding: "10px" }}>Admin Login</button>
          ) : (
            <div>
              <div style={{ background: "#1e293b", padding: "15px", marginBottom: "20px" }}>
                <h3>Settings</h3>
                <input placeholder="WhatsApp Number" onChange={e => setWhatsapp(e.target.value)} style={{ width: "100%", padding: "8px", background: "#020617", color: "#fff" }} />
              </div>
              <div style={{ background: "#1e293b", padding: "15px" }}>
                <h3>Add Product</h3>
                <input placeholder="Name" onChange={e => setNewProd({...newProd, name: e.target.value})} style={{ width: "100%", marginBottom: "5px", padding: "8px", background: "#020617", color: "#fff" }} />
                <input placeholder="Price" onChange={e => setNewProd({...newProd, price: e.target.value})} style={{ width: "100%", marginBottom: "5px", padding: "8px", background: "#020617", color: "#fff" }} />
                <button onClick={addProd} style={{ width: "100%", background: "#059669", padding: "10px", color: "#fff", border: "none" }}>Add</button>
              </div>
              <h3>Current Products</h3>
              {products.map(p => (
                <div key={p.id} style={{ display: "flex", justifyContent: "space-between", background: "#334155", padding: "10px", marginTop: "5px" }}>
                  {p.name} <button onClick={() => delProd(p.id)} style={{ background: "red", color: "#fff", border: "none" }}>Delete</button>
                </div>
              ))}
            </div>
          )}
